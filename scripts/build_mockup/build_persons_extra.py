...
entity_type == 'person'. persons.html reads it directly (no
hand-curated override object sits in front of it, unlike work.html's
WORKS/place.html's PLACES) so any ?reg=… link to persons.html resolves
to real metadata instead of a blank page.
...
def load_person_wikidata() -> dict:
    ...
    of this writing, so no person in PERSONS_EXTRA carries a wd value.
    (Until 2026-09-07, mockup/person.html — an unreferenced legacy page,
    since deleted — carried its own small hand-curated demo entry,
    Dickens/Q5686, but that lived only in that page's own script and was
    never part of PERSONS_EXTRA.) This loader exists so the "already has an
    authority link" check below is forward-compatible: if a curated overlay
    is added later, bio_search_links() picks it up
...
def main() -> None:
    ...
    # Emit one entry per person. Unlike work.html/place.html, persons.html
    # has no hand-curated override object in front of this data (that
    # pattern — ALL_PERSONS = Object.assign({}, PERSONS_EXTRA, PERSONS) —
    # lived only in mockup/person.html, an unreferenced legacy page deleted
    # 2026-09-07; see docs/data-model/detail-page-redundancy.md). Emitting
    # every person here is what lets EntityRefs (mockup/js/entity-refs.js)
    # see the full register from any page.
    ...
    with open(OUT, "w", encoding="utf-8") as f:
        ...
        f.write("// One entry per person in PERSON-REGISTER. persons.html reads this\n")
        f.write("// directly -- unlike work.html/place.html, there is no hand-curated\n")
        f.write("// override object in front of it.\n")
