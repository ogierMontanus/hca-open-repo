...
| [`place-typology.md`](place-typology.md) | **Draft, awaiting editorial sign-off.** Proposed place-type taxonomy for `data/raw/SV14_places.xml` (481 places), derived bottom-up from place names and the existing GeoNames-derived `type` field. Two variants side by side: the original 11-category breakdown (§A–D) and a requested 5-category compression with a GeoNames Feature Class/Code mapping (§E) — buildings/institutions/monuments/parks merged into one bucket, islands folded into landforms. §E also reports a zero-shot name-pattern test against the much larger 2508-place STED-REGISTER (`data/normalized/entities.csv`): ~70% estimated accuracy, with the specific German toponym and pre-1948 Danish spelling failure modes that drive the error rate. §F documents the final, editorially approved 6-category variant (with GeoNames + Wikidata mappings per category, plus the Category 2-vs-4 administrative-suffix rule and the canal-to-Vandområder exception) |
| [`place-categorization-copilot-prompt.md`](place-categorization-copilot-prompt.md) | Copilot/bot prompt for classifying place names against the final 6-category taxonomy from `place-typology.md` §F — category definitions, boundary-case rules, and a worked 80-place sample with filled-in classifications |
