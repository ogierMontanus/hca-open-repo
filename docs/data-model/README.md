...
| [`place-typology.md`](place-typology.md) | **Draft, awaiting editorial sign-off.** Proposed place-type taxonomy for `data/raw/SV14_places.xml` (481 places), derived bottom-up from place names and the existing GeoNames-derived `type` field. Two variants side by side: the original 11-category breakdown (§A–D) and a requested 6-category compression with a GeoNames Feature Class/Code mapping (§E) — buildings/institutions/monuments merged into one bucket, islands folded into landforms |
